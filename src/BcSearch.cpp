...
void BcSearch::think() {
	...
	for (int i = 1; i <= max_depth; ++i) {
		...
		if (fixed_depth == 0 && max_depth > 1)
			if (fixed_time == 1) {
				if (BcTime::GetTime() >= start_time + max_time) {
					stop_search = true;
					return;
				}
			} else if (BcTime::GetTime() >= start_time + max_time / 4) {
				stop_search = true;
				return;
			}
		...
		printf("%d %d %d %d ", i, x, (BcTime::GetTime() - start_time) / 10, nodes);
 ...
 */
int BcSearch::Search(int alpha, int beta, int depth) {
	if (ply && reps2()) {
		return 0;
	}
 ...
 */
int BcSearch::CaptureSearch(int alpha, int beta) {
	nodes++;
 ...
 */
int BcSearch::ReCaptureSearch(int a, const int sq) {
	int b;
	int c = 0;
	int t = 0;
	int score[12];
 ...
 */
void BcSearch::Sort(const int from) {
	move g;
 ...
 */
void BcSearch::CheckUp() {
	if ((BcTime::GetTime() >= stop_time || (max_time < 50 && ply > 1))
			&& fixed_depth == 0 && ply > 1) {
		stop_search = true;
		longjmp(env, 0);
	}
 ...
 */
void BcSearch::SetHashMove() {

	for (int x = first_move[ply]; x < first_move[ply + 1]; x++) {
		if (move_list[x].start == hash_start
				&& move_list[x].dest == hash_dest) {
