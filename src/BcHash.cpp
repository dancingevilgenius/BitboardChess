...
//hashp *hashpos[2];  // moved to inside new class HashP
...
void BcHashP::RandomizeHash() {
	...
	hashpos[0] = new BcHashP[MAXHASH];
	hashpos[1] = new BcHashP[MAXHASH];
}
/*
 ...
 */
int BcHashP::Random(const int x) {
	return rand() % x;
}
/*
 ...
 */
void BcHashP::AddHash(const int s, const BcMove m) {
	BcHashP *ptr = &hashpos[s][currentkey];
 ...
 */
void BcHashP::AddKey(const int s, const int p, const int x) {
 ...
 */
bool BcHashP::LookUp(const int s) {
